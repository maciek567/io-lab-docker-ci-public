...
image:
	docker build . -t "$(PREFIX_DOCKER)/$(REPO_NAME)" \
  	...
  	&& docker tag "$(PREFIX_DOCKER)/$(REPO_NAME):latest" "$(PREFIX_DOCKER)/$(REPO_NAME):$(TAG)"
	
push: image
	docker push "$(PREFIX_DOCKER)/$(REPO_NAME):latest"
	docker push "$(PREFIX_DOCKER)/$(REPO_NAME):$(TAG)"
